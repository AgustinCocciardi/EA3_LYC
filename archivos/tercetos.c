...
/** Agrega un terceto a la lista de tercetos. Si se quiere guardar solo una constante o variable, mandar SIN_OPERADOR en
...
void escribirTerceto(int indice, int posicion, int valor){
...
void guardarTercetos(){
	...
	for(i = 0; i <= ultimo_terceto; i++){
		...
		//Si el operador es SIN_OPERADOR, es solo una ID o CTE que ya optimizamos, lo ignoro
		if(lista_terceto[i].operador != SIN_OPERADOR){
			//Escribo indice
...
void optimizarTercetos(){
	...
	for(i=0; i<MAX_TERCETOS; i++){
		...
		if(lista_terceto[i].op1 >= OFFSET){
			...
			if(lista_terceto[ref].operador == SIN_OPERADOR) //Si el referenciado solo tiene una ID o CTE
